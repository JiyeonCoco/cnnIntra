...
class ConvNet(nn.Module):
    def __init__(self, in_layers, num_classes, block_size, group_num):
        ...
        self.conv2 = nn.Conv2d(in_channels=16 * group_num, out_channels=32 * group_num, kernel_size=3, stride=1, padding=1, groups=group_num)
        self.bn2 = nn.BatchNorm2d(32 * group_num)
        self.pool2 = nn.MaxPool2d(kernel_size=2, stride=2)

        # # parameter 개수가 너무 많아서 속도 저하됨. parameter를 줄여주기 위한 conv. 값 변화는 없음
        self.conv3 = nn.Conv2d(in_channels=32 * group_num, out_channels=64, kernel_size=1, stride=1, padding=0, groups=1)
